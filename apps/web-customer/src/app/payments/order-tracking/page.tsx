...
export default function OrderTracking() {
  ...
  useEffect(() => {
    ...
    setOrderIds(ids)
    if (ids[0]) {
      ordersApi.tracking(ids[0]).then(setTracking).catch(() => {}).finally(() => setLoaded(true))
    } else {
      setLoaded(true)
    }
  }, [])
            ...
            <Card className="card-surface">
              <CardContent className="p-6 sm:p-8">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
                  <div>
                    <h3 className="text-xl font-bold text-gray-900 dark:text-white">Order #{orderIds[0] ?? "—"} Progress</h3>
                    <p className="text-xs text-gray-500 mt-1">Real-time order status from the SahulatKar ledger</p>
                  </div>
                    ...
                    return (
                      <div key={step.code} className="relative group">
                        ...
                        <div className="space-y-1 pl-2">
                          <h4 className={`text-sm font-bold ${isActive ? "text-orange-500" : isCompleted ? "text-gray-800 dark:text-gray-200" : "text-gray-400"}`}>
                            {step.title}
                          </h4>
                        </div>
                      </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between mb-4 border-b border-gray-100 dark:border-white/5 pb-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <h3 className="text-sm font-extrabold text-gray-900 dark:text-white uppercase tracking-wider">Latest Tracking Event</h3>
                    </div>
                  </div>
