...
async def test_get_admin_user_detail_returns_404_for_unknown_user(client: AsyncClient, test_admin):
    _, admin_token = test_admin
    r = await client.get("/api/v1/admin/users/999999", headers=_auth(admin_token))
    assert r.status_code == 404
    assert r.json()["detail"] == "USER_NOT_FOUND"


async def test_get_user_risk_alias_route(client: AsyncClient, test_user, test_admin):
    user, _ = test_user
    _, admin_token = test_admin
    r = await client.get(f"/api/v1/admin/users/{user.id}/risk", headers=_auth(admin_token))
    assert r.status_code == 200
    body = r.json()
    assert body["user_id"] == user.id
    assert "items" in body
...
async def test_get_user_contracts_route(client: AsyncClient, test_user, test_admin):
    user, _ = test_user
    _, admin_token = test_admin
    r = await client.get(f"/api/v1/admin/users/{user.id}/contracts", headers=_auth(admin_token))
    assert r.status_code == 200
    body = r.json()
    assert body["user_id"] == user.id
    assert "wakalah" in body
    assert "murabaha" in body
