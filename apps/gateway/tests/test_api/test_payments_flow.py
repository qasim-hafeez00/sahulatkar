...
async def test_down_payment_idempotency_key_returns_cached_response(client: AsyncClient, test_user, db_session):
    from sk_shared.models.order import Order
    ...
    payload = {"order_id": order.id, "method": "raast", "amount_pkr": "3000.00"}
    headers = {**_auth(token), "X-Idempotency-Key": "idem-down-1"}

    r1 = await client.post("/api/v1/payments/down-payment", json=payload, headers=headers)
    assert r1.status_code == 200

    r2 = await client.post("/api/v1/payments/down-payment", json=payload, headers=headers)
...
async def test_installment_payment_idempotency_key_returns_cached_response(client: AsyncClient, test_user, db_session):
    from sk_shared.models.order import Order
    from sk_shared.models.payment import Loan, Installment
    ...
    headers = {**_auth(token), "X-Idempotency-Key": "idem-inst-1"}
    payload = {"method": "raast", "amount_pkr": "2000.00"}
    r1 = await client.post(f"/api/v1/payments/installment/{installment.id}/pay", json=payload, headers=headers)
    assert r1.status_code == 200

    r2 = await client.post(f"/api/v1/payments/installment/{installment.id}/pay", json=payload, headers=headers)
    assert r2.status_code == 200
    assert r1.json()["payment_id"] == r2.json()["payment_id"]
