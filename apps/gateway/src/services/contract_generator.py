...
class ContractGeneratorService:
    ...
    def _build_pdf(self, title: str, lines: list[str], clauses: list[str] = None) -> bytes:
        # We need a local temp path for ReportLab to write to before uploading
        local_temp_dir = Path("./tmp/pdf_gen")
        local_temp_dir.mkdir(parents=True, exist_ok=True)
        temp_file = local_temp_dir / f"tmp-{uuid.uuid4()}.pdf"
        c = canvas.Canvas(str(temp_file))
        c.setTitle(title)
        ...
        c.drawRightString(550, 50, f"Page 1 | Integrity Hash: Verification Required")
    ...
    async def generate_wakalah(self, user_id: int, req: WakalahGenerateRequest, redis) -> WakalahAgreement:
        ...
        if existing is not None:
            await ContractSignerService.issue_signing_otp(self.db, redis, "wakalah", existing.id)
            return existing
        ...
        await ContractSignerService.issue_signing_otp(self.db, redis, "wakalah", contract.id)
        return contract
    ...
    async def generate_murabaha(self, user_id: int, req: MurabahaGenerateRequest, redis) -> MurabahaContract:
        ...
        if existing is not None:
            await ContractSignerService.issue_signing_otp(self.db, redis, "murabaha", existing.id)
            return existing
        ...
        await ContractSignerService.issue_signing_otp(self.db, redis, "murabaha", contract.id)
        return contract
